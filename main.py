...
EMOJI_MAP = {
    "AC": "<:ac:1545977298912673855>",
    "WA": "<:wa:1545975538265489508>",
    "RE": "<:re:1545975583509319830>",
    "CE": "<:ce:1545975623028310036>",
    "TLE": "<:tl:1545975668129403011>",
    "MLE": "<:ml:1545975704448143501>"
}
...
class AtCoderBot(discord.Client):
    ...
    def create_status_embed(self, d, target):
        ...
        now = datetime.now(JST)
        wd_ja = ["月", "火", "水", "木", "金", "土", "日"]
        embed.set_footer(text=f"{now.strftime(f'%Y-%m-%d %H:%M:%S')} 時点")
        return embed
...
async def delete(interaction: discord.Interaction, atcoder_id: str):
    ...
    if key in bot.user_data:
        ...
        await interaction.followup.send(f"🗑️ `{atcoder_id}` さんの登録を削除しました。")
...
async def notice_delete(interaction: discord.Interaction):
    ...
    if gid in bot.news_config:
        ...
        await interaction.followup.send("🗑️ 告知登録を削除しました。")
